...
```{r compare, fig.cap="the hardest time in Belgium", fig.path = "figures/"}
compare <- format %>% 
  as.data.frame() %>%
...
```{r facet-1, out.height = "80%", fig.cap = "Difference age groups of death cases in three reigions" }
...
Compare with United State and China which are the country got serious epidemic, we calculate the death rate and recovery rate. Figure \@ref(fig:dt) shows that Belgium got the second highest death rate in April.

```{r dt, fig.cap = "date compared"}
dtable <- DT::datatable(second, options = list(pageLength = 5))
dtable
```
